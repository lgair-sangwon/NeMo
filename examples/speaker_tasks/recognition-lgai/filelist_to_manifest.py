...
DURATIONS = sorted([1.5, 2, 3], reverse=True)
MIN_ENERGY = 0.01
CWD = os.getcwd()
MAX_WORKERS = int(os.environ.get("MAX_WORKERS", "8"))


def filter_manifest_line(manifest_line, id=-1):
    split_manifest = []
    ...
    endname = os.path.join(*os.path.splitext(audio_path)[0].split("/")[id + 1 :])
    to_path = os.path.join(CWD, 'segments', label)
    to_path = os.path.join(to_path, endname)
...
def write_file(name, lines, idx):
    with open(name, 'w', encoding='utf-8') as fout:
        for i in tqdm(idx, total=len(idx), desc="writing..."):
            dic = lines[i]
            json.dump(dic, fout)
            fout.write('\n')
    print("wrote", name)
...
def read_file(filelist, id=-1):
    ...
    with open(filelist, 'r') as fo:
        ...
        for line in lines:
            line = line.strip()
            dataset, speaker = line.split('/')[0], line.split('/')[id]
            speaker = list(speaker)
            speaker = ''.join(speaker)
            meta = {"audio_filepath": line, "offset": 0, "duration": None, "label": f"{dataset}-{speaker}"}
...
def get_duration(json_line):
    ...
    if dur is None:
        ...
        json_line['duration'] = sf.info(wav_path).duration
    return json_line
...
def main(filelist, manifest, id, out, split=False, create_segments=False, min_count=10):
    ...
    if filelist:
        ...
        lines = process_map(get_duration, lines, chunksize=100, desc="get_duration", max_workers=MAX_WORKERS)
    ...
    if create_segments:
        ...
        lines = process_map(
            partial(filter_manifest_line, id=id), lines, chunksize=100, desc="filter_manifest", max_workers=MAX_WORKERS
        )
        temp = []
        for line in lines:
            temp.extend(line)
